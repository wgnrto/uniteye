...
* **Session quality** — calibration reports a holdout-RMSE confidence verdict (recalibrate advice when poor; `HomulerGazeCalibration.LastHoldoutRmseCm` for host-game gating), profiles record whether the user **wore glasses** (load warns on mismatch — worth ~1 cm), and `BinocularIrisDisagreement` exposes a per-frame quality proxy. On the direction backbones,
`HomulerGaze.GazeModelConfidence` adds a *live* per-frame confidence read straight off the model's own
output distribution — it widens the AOI probability ellipse, down-weights drift anchors and gates
calibration capture when the model is having a bad frame (see [docs/GAZE-BACKBONES.md](docs/GAZE-BACKBONES.md)).
