...
**Feature Augmentation** is disabled by default. When enabled, it adds deterministic, bounded zero-mean jitter to the numerical gaze-model features during fitting only; its scale is derived from each training feature's standard deviation. It preserves target labels and never touches held-out RMSE or evaluation samples. Do not use image flips, crops, or rotations here: calibration learns from feature vectors and those transformations do not preserve a screen-target label. Compare a no-augmentation and augmented run with the independent [evaluation](#evaluation) sequence—especially its corner and edge RMSE—and retain augmentation only when it improves an unseen session.
