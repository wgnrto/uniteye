...
* Predictions compressing toward the centre / not reaching the corners is largely a **calibration and filtering** issue, addressed by two changes: every backbone now feeds a polynomial of its raw gaze to the calibration (see [Gaze models](#gaze-models-backbones)), and the default One-Euro filter was re-tuned — its old `mincutoff`/`beta` (0.001) over-smoothed ~1000×, so the dot lagged and undershot fast glances. If the dot still feels sluggish or laggy, raise **Mincutoff** (and **Beta**) in the [Gaze UI](#gaze-ui), or set **Filtering → None** to see the unfiltered gaze. **Recalibrate** after updating — the EyeMU feature vector changed, so older EyeMU calibration files fall back to raw gaze.
